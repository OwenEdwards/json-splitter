...
def json_splitter(file_name, mb_per_file):
    ...
    change_severity_of_advisory_issues_val = change_severity_of_advisory_issues.instate(['selected'])

    # check that file is larger than max size
    if(not instanceID_to_note_val and not change_severity_of_advisory_issues_val):
    ...
    for i in range(0,num_files):
        ...
        for n in range(starts[i],starts[i+1]):
            ...
            if(change_severity_of_advisory_issues_val):
                if "[ADVISORY]" in data[n]["description"]:
                    data[n]["severity"] = "low";
...
# Set up the window
window = tk.Tk()
window.title("AMP to Platform JSON Splitter" + " " + appVersion)
...
ent_filesize = ttk.Spinbox(master=frm_entry, width=4, from_=1, to=1000, increment=10, textvariable=track_filesize)
...
track_change_severity_of_advisory_issues = tk.StringVar()
change_severity_of_advisory_issues = ttk.Checkbutton(master=frm_entry, text='Change severity of ADVISORY issues', variable=track_change_severity_of_advisory_issues)
change_severity_of_advisory_issues.state(['selected']) # default this to "selected" (checked)
...
change_severity_of_advisory_issues.grid(row=0, column=5, sticky="e", padx=20)
...
track_change_severity_of_advisory_issues.trace_add("write", clear_results)
